...
async function main() {
  ...
  // Services
  const servicesBlock = pageOverrides["services"]?.blocks.find(
    (b) => b.kind === "cards",
  ) as any;
  if (servicesBlock) {
    await insertItems(
  ...
  // Team
  const teamBlock = pageOverrides["team"]?.blocks.find(
    (b) => b.kind === "people",
  ) as any;
  if (teamBlock) {
    await insertItems(
  ...
  // Customers (Cards)
  const customersBlock = pageOverrides["customers"]?.blocks.find(
    (b) => b.kind === "cards",
  ) as any;
  if (customersBlock) {
    await insertItems(
  ...
  // Pricing
  const pricingBlock = pageOverrides["pricing"]?.blocks.find(
    (b) => b.kind === "pricing",
  ) as any;
  if (pricingBlock) {
    await insertItems(
