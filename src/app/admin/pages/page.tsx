...
function PagesAdmin() {
  ...
  const refresh = () => queryClient.invalidateQueries({ queryKey: cmsPagesQuery.queryKey });
  ...
  return (
    <div>
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        <Button className="glow-ring" onClick={() => setDraft({ published: false })}>
      ...
      <div className="mt-6 space-y-2">
        {(data ?? []).map((page) => (
          <div key={page.id} className="surface-card flex items-center gap-3 p-4">
            <button type="button" className="flex-1 text-left" onClick={() => setDraft(page)}>
              <p className="font-medium">{page.title}</p>
              <p className="text-muted-foreground font-mono text-xs">/{page.slug}</p>
            </button>
            <span
              className={page.published ? 'text-primary text-xs' : 'text-muted-foreground text-xs'}
            >
        ...
        {!data?.length && !draft && (
          <p className="text-muted-foreground text-sm">No CMS pages yet — create your first one.</p>
        )}
      </div>
    </div>
  );
