...
export function ProductsIndex({ dynamicProducts = [] }: { dynamicProducts?: ContentItem[] }) {
  return (
    <div>
        ...
        <div className="mx-auto max-w-4xl px-5 text-center">
          ...
          <Reveal variant="up" delay={150}>
            <p className="text-muted-foreground mx-auto mt-6 max-w-2xl text-lg">
              Nazexa DB Design turns database modelling into a visual, validated workflow. Nazexa
              DEV Tools puts the utilities you reach for every day into one fast workspace.
            </p>
          </Reveal>
        </div>
      </section>
        ...
        <div className="grid gap-6 lg:grid-cols-2">
          {dynamicProducts.map((p, i) => (
            <Reveal key={p.id} delay={i * 110}>
              <article
                className="surface-card hover-lift group flex h-full flex-col overflow-hidden p-6"
                style={{
                  ['--primary' as string]: p.tone ? `var(--${p.tone})` : `var(--brand-1)`,
                }}
              >
                {p.slug === 'db-design' && <ErdVisual />}
                {p.slug === 'dev-tools' && <DevToolsVisual />}
                {p.image_url && p.slug !== 'db-design' && p.slug !== 'dev-tools' && (
                  <div className="border-border bg-muted/30 -mx-6 -mt-6 mb-6 overflow-hidden border-b">
                ...
                <p className="text-primary mt-1 text-sm font-medium">{p.subtitle}</p>
                {p.body && (
                  <div
                    className="text-muted-foreground prose prose-sm dark:prose-invert prose-p:leading-relaxed prose-li:my-0.5 mt-4 text-sm"
                  ...
                  {p.link_url && (
                    <Button asChild className="glow-ring">
                      <Link href={p.link_url}>
                        {p.link_label || 'Explore'} <ArrowRight className="ml-1.5 h-4 w-4" />
                      </Link>
                    </Button>
                  )}
        ...
        <Reveal variant="zoom">
          <div className="surface-card glow-ring relative overflow-hidden p-10 text-center sm:p-16">
            ...
            <p className="text-muted-foreground mx-auto mt-4 max-w-xl">
              Both products are free to try, and they share the same account when you are ready for
              the other.
            </p>
